...
namespace logging {
  ...
  void print_help(const char *name) {
    std::cout
      ...
      << "    --creds username password | set persistent admin credentials"sv << std::endl
      << "    --set key=value [...]     | persist config options to the config file"sv << std::endl
      << "    --version                 | print the version of sunshine"sv << std::endl
      << std::endl
      << "    flags"sv << std::endl
      << "        -0 | Read Moonlight pairing PIN from stdin"sv << std::endl
